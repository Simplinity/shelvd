...
function drawFieldPair(ctx: DrawContext, label1: string, value1: string | undefined | null, label2: string, value2: string | undefined | null) {
  ...
  if (value1 && value1.trim()) {
    ...
    ctx.page.drawText(safeText(value1), { x: ctx.margin + labelWidth, y: ctx.y, size: ctx.fontSize, font: ctx.regular, color: ctx.textColor })
  }
  ...
  if (value2 && value2.trim()) {
    ...
    ctx.page.drawText(safeText(value2), { x: x2 + labelWidth, y: ctx.y, size: ctx.fontSize, font: ctx.regular, color: ctx.textColor })
  }
...
export async function generateCatalogSheet(data: BookPdfData, paperSize: PaperSize): Promise<Uint8Array> {
  ...
  // ═══ HEADER: Title ═══
  // Title
  const titleLines = wrapText(bold, safeText(data.title), ctx.maxTextWidth, titleSize)
  ...
  // Subtitle
  if (data.subtitle) {
    ctx.page.drawText(safeText(data.subtitle), { x: ctx.margin, y: ctx.y, size: subtitleSize, font: italic, color: ctx.labelColor })
    ctx.y -= subtitleSize * 1.5
  }
  
  // Contributors line
  if (data.contributors.length > 0) {
    const contribLine = data.contributors.map(c => `${safeText(c.name)} (${c.role})`).join(', ')
  ...
  if (data.provenance.length > 0) {
    ...
    for (const p of data.provenance) {
      ...
      ctx.page.drawText(safeText(line), { x: ctx.margin, y: ctx.y, size: ctx.fontSize, font: ctx.regular, color: ctx.textColor })
      ctx.y -= ctx.lineHeight
      if (p.notes) {
        ctx.page.drawText(safeText(p.notes), { x: ctx.margin + 10, y: ctx.y, size: ctx.fontSize - 1, font: ctx.italic, color: ctx.labelColor })
        ctx.y -= ctx.lineHeight
      }
    }
  }
  ...
  if (footerY > 8) {
    ...
    ctx.page.drawText(safeText(footerText), {
      x: ctx.margin,
      y: footerY,
    ...
    if (data.owner_name) {
      ...
      ctx.page.drawText(safeText(data.owner_name), {
