...
export const CHANGELOG: ChangelogRelease[] = [
  {
    ...
    changes: [
      ...
      { type: 'added', text: 'OpenGraph + Twitter Card images — dynamic Swiss Design OG images for homepage, blog articles, and wiki pages' },
      { type: 'added', text: 'Global error page — catches crashes with a friendly message and retry button' },
      { type: 'added', text: 'Global 404 page — "Ex Libris Nobody" with bibliophile humor' },
      { type: 'added', text: 'Loading indicator — pulsing red bar between page navigations' },
      { type: 'added', text: 'Toast notifications — all 11 browser alert() calls replaced with non-blocking Sonner toasts' },
      { type: 'added', text: 'Save confirmation toast after editing or adding a book' },
      { type: 'added', text: 'Unsaved changes indicator — browser tab shows ● when you have unsaved edits' },
      { type: 'added', text: 'Duplicate warning on add — type a title and see if you already own it, with links to existing copies' },
      { type: 'added', text: 'Skip-to-content accessibility link for keyboard navigation' },
      { type: 'added', text: 'JSON-LD structured data on homepage (Organization + SoftwareApplication), blog, and about page' },
      { type: 'added', text: 'Dynamic browser tab titles — every page shows its name, book detail shows the book title, collection shows count' },
      { type: 'added', text: 'Scroll-to-top button on long book lists' },
      { type: 'improved', text: 'Alt text on all cover images — book title instead of empty, for screen readers' },
      { type: 'improved', text: 'Grid view sort dropdown — 8 sort fields with database-side sorting across all pages' },
      { type: 'improved', text: 'Database performance — 5 sort indexes + 3 composite indexes + lazy loading images' },
      { type: 'added', text: 'Favicon, apple-touch-icon, web manifest, robots.txt, sitemap.xml (65 pages)' },
    ],
  },
  {
    version: '0.23.0',
